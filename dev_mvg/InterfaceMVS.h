...
//////////////////////////////////////
///////Save obj as txt by asano///////

template<typename _Tp>
bool Save_Text(const _Tp& obj, const std::string& fileName, uint32_t version = MVSI_PROJECT_VER) {
	// open the output stream
	std::ofstream stream(fileName, std::ios::out); //normal file output
	if (!stream.is_open())
		return false;
	// write header
	if (version > 0) {
		// save project ID
		stream<<MVSI_PROJECT_ID << std::endl;
		// save project version
		stream<<version << std::endl;
	}
	// save the current state as text
	ARCHIVE::ArchiveSave serializer(stream, version);
	serializer& obj;
	return true;
}
